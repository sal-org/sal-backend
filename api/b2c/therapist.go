...
func TherapistProfile(w http.ResponseWriter, r *http.Request, body map[string]string) {
	...
	therapist, status, ok := DB.SelectSQL(CONSTANT.TherapistsTable, []string{"first_name", "last_name", "pronoun", "total_rating", "average_rating", "photo", "video", "price", "multiple_sessions", "education", "experience", "therapeutic_approach", "about", "slot_type"}, map[string]string{"therapist_id": body["therapist_id"]})
	if !ok {
	...
	counsellor["price"] = therapist[0]["price"]
	counsellor["multiple_sessions"] = therapist[0]["multiple_sessions"]
