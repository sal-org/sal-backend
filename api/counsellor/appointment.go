...
func AppointmentsUpcoming(w http.ResponseWriter, r *http.Request) {
	...
	appointments, status, ok := DB.SelectProcess("select * from "+CONSTANT.AppointmentsTable+" where counsellor_id = ? and status in ("+CONSTANT.AppointmentToBeStarted+", "+CONSTANT.AppointmentStarted+") order by date desc", r.FormValue("counsellor_id"))
	if !ok {
...
func AppointmentsPast(w http.ResponseWriter, r *http.Request) {
	...
	/*appointments, status, ok := DB.SelectSQL(CONSTANT.AppointmentsTable, []string{"*"}, map[string]string{"counsellor_id": r.FormValue("counsellor_id"), "status": CONSTANT.AppointmentCompleted, "order by date": "desc"})
	if !ok {
		UTIL.SetReponse(w, status, "", CONSTANT.ShowDialog, response)
		return
	}*/
