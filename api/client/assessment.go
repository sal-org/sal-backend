...
func AssessmentHistory(w http.ResponseWriter, r *http.Request) {
	...
	assessmentResults, status, ok := DB.SelectProcess("select final_score, created_at from "+CONSTANT.AssessmentResultsTable+" where user_id = ? and assessment_id = ? and status = "+CONSTANT.AssessmentResultActive+" order by created_at desc", r.FormValue("client_id"), r.FormValue("assessment_id"))
	if !ok {
		UTIL.SetReponse(w, status, "", CONSTANT.ShowDialog, response)
		return
	}
