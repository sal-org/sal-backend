...
func LoadClientRoutes(router *mux.Router) {
	...
	clientRoutes.HandleFunc("", CreateUserHandler).Methods("POST")

	clientRoutes.HandleFunc("/assessment", CreateUserHandlerForAssessment).Methods("POST")
...
func CreateUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "/listener/get":
		ListenerProfile(w, r, decrypted)
	case "/listener/slots":
		ListenerSlots(w, r, decrypted)
	case "/listener/order":
		ListenerOrderCreate(w, r, decrypted)
	case "/listener/paymentcomplete":
		ListenerOrderPaymentComplete(w, r, decrypted)
	case "/mood/add":
		MoodAdd(w, r, decrypted)
	case "/mood/history":
		MoodHistory(w, r, decrypted)
	case "/mood/content":
		ListMoodContent(w, r, decrypted)
	case "/notification/get":
		NotificationsGet(w, r, decrypted)
	case "/family_member/delete":
		DeleteAccountForFamilyMember(w, r, decrypted)
	case "/profile/get":
		ProfileGet(w, r, decrypted)
	case "/profile/add":
		ProfileAdd(w, r, decrypted)
	case "/corporate/profile/add":
		ProfileAddForCor(w, r, decrypted)
	case "/profile/update":
		ProfileUpdate(w, r, decrypted)
	case "/corporate_get_relation/add":
		RelativeProfileAdd(w, r, decrypted)
	case "/corporate_get_relation/get":
		GetRelativeProfile(w, r, decrypted)
	case "/search/get":
		ListSearch(w, r, decrypted)
	case "/webinar/get":
		WebinarList(w, r, decrypted)
	case "/webinar/order":
		WebinarOrderCreate(w, r, decrypted)
	case "/corporate_search/get":
		ListSearchForCorporate(w, r, decrypted)
	case "/corporate_inperson_search/get":
		ListSearchForCorporateInPerson(w, r, decrypted)
	case "/therapist/get":
		TherapistProfile(w, r, decrypted)
	case "/therapist/slots":
		TherapistSlots(w, r, decrypted)
	case "/in_person_therapist/slots":
		InPersonTherapistSlots(w, r, decrypted)
	case "/therapist/order":
		TherapistOrderCreate(w, r, decrypted)
	case "/therapist/paymentcomplete":
		TherapistOrderPaymentComplete(w, r, decrypted)
	case "/therapist/getHashData":
		GenerateHashForPayment(w, r, decrypted)
	case "/restore-user-account":
		RestoreUserProfile(w, r, decrypted)
	case "/corporateCounsellor/order":
		CorporateCounsellorOrderCreate(w, r, decrypted)
	case "/corporateCounsellor/paymentcomplete":
		CorporateCounsellorOrderPaymentComplete(w, r, decrypted)
	case "/inperson_corporateCounsellor/order":
		InPersonCorporateCounsellorOrderCreate(w, r, decrypted)
	case "/inperson_corporateCounsellor/paymentcomplete":
		InPersonCorporateCounsellorOrderPaymentComplete(w, r, decrypted)
	default:
...
// CreateUserHandler handles user creation with detailed error logging
func CreateUserHandlerForAssessment(w http.ResponseWriter, r *http.Request) {
	...
	decrypted, _ := UTIL.DecryptPayloadForAssessment(encryptedPayload.Payload, CONSTANT.ENCRYPTION_SECRET_KEY, CONSTANT.ENCRYPTION_SECRET_IV)
