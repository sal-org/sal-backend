...
func ListSearch(w http.ResponseWriter, r *http.Request, body map[string]string) {
	...
	counsellorFirstList := []map[string]string{}
	counsellorSecondList := []map[string]string{}
...
func ListSearchForCorporate(w http.ResponseWriter, r *http.Request, body map[string]string) {
	w.Header().Set("Content-Type", "application/json")
...
func ListSearchForCorporateInPerson(w http.ResponseWriter, r *http.Request, body map[string]string) {
	w.Header().Set("Content-Type", "application/json")
	...
	if client[0]["email"] == "anand.shah@clovemind.com" || client[0]["email"] == "shivam.tiwari@clovemind.com" || client[0]["email"] == "karishma.vora@clovemind.com" {
		...
		counsellors, status, ok = DB.SelectProcess(mYSQL + " limit " + strconv.Itoa(CONSTANT.CounsellorsListPerPageClient) + " offset " + strconv.Itoa((UTIL.GetPageNumber(body["page"])-1)*CONSTANT.CounsellorsListPerPageClient))
		if !ok {
			UTIL.SetReponse(w, status, "", CONSTANT.ShowDialog, response)
			return
		}
