...
func AvailabilityGet(w http.ResponseWriter, r *http.Request) {
	...
	availability, status, ok := DB.SelectProcess("select * from "+CONSTANT.SchedulesTable+" where counsellor_id = ?", r.FormValue("therapist_id"))
	if !ok {
...
func AvailabilityUpdate(w http.ResponseWriter, r *http.Request) {
	...
	for _, day := range body {
		DB.UpdateSQL(CONSTANT.SchedulesTable, map[string]string{"counsellor_id": r.FormValue("therapist_id"), "weekday": day["weekday"]}, day)
	}

	// get all dates for therapist and group by weekday
	datesByWeekdays := map[int][]string{}
	availabileDates, status, ok := DB.SelectProcess("select date from "+CONSTANT.SlotsTable+" where counsellor_id = ?", r.FormValue("therapist_id"))
	if !ok {
		UTIL.SetReponse(w, status, "", CONSTANT.ShowDialog, response)
		return
	}
